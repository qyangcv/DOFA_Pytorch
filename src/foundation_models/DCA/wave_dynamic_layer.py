...
class TransformerWeightGenerator(nn.Module):
    def __init__(self, input_dim, output_dim, embed_dim, num_heads=4, num_layers=1):
        ...
        self.fc_weight = nn.Linear(input_dim, output_dim) # output_dim = patch_size * patch_size * embed_dim = 16*16*768
    ...
    def forward(self, x):
        ...
        pos_wave = x # input x is positional-embedded waves x has shape [9,128]
        x = torch.cat([self.weight_tokens, pos_wave], dim=0) # w_t[128,128] + x[9,128] -> x[128+9,128]
        x = torch.cat([x, self.bias_token], dim=0) # x[128+9,128] + b_t[1,128] -> x[128+9+1,128]
        transformer_output = self.transformer_encoder(x) # self_attention, [138,128] -> [138,128]
        weights = self.fc_weight(transformer_output[self.wt_num : -1] + pos_wave) # [128+9+1,128] -> [9,128] -> [9,16*16*768]
        bias = self.fc_bias(
            transformer_output[-1]
        )  # Using the last output to generate bias, [128] -> [768]
        return weights, bias
...
class Dynamic_MLP_OFA(nn.Module):
    ...
    def forward(self, img_feat, wvs):
        """
        img_feat: [1,9,224,224]
        wvs: [9]
        return: [1,768,224,224]
        """
        # origin type wvs is OmegaConf.ListConfig
        if not isinstance(wvs, torch.Tensor): 
            wvs = torch.tensor(wvs, device=img_feat.device)
            
        inplanes = wvs.size(0) # inplanes is channels of wavelength
        
        waves = get_1d_sincos_pos_embed_from_grid_torch(self.wv_planes, wvs * 1000) # [9] -> [9,128]
        waves = self.fclayer(waves) # [9,128] -> [9,128]
        weight, bias = self._get_weights(waves)  # weight: [9,16*16*768], bias: [768]

        # Fix bug        
        dynamic_weight = weight.view(inplanes, self.kernel_size, self.kernel_size, self.embed_dim) # [9,16,16,768]
        dynamic_weight = dynamic_weight.permute([3,0,1,2]) # [768,9,16,16]
        
        if bias is not None:
            bias = bias.view([self.embed_dim]) * self.scaler # [768]

        weights = dynamic_weight * self.scaler # [768,9,16,16]
